...
module RUN_BSON
  module Grow
    ...
    def unfinish! # Backup past terminating null bytes
      @b_pos ||= [0]
      @cursor = @str.size - @b_pos.size # RUN_BSON::BSON_CODER.serialize may not restore @cursor
      self
    end
    ...
    def push_doc(bson) # Appends BSON doc with correct key finished
      @a_index ||= [0]
      @b_pos ||= [0]
      put(RUN_BSON::BSON_RUBY::OBJECT, @str.size - @b_pos.size)
      put_binary(@a_index[-1].to_s)
      put(0)
      @a_index[-1] += 1
      put_binary(bson.to_s)
    ...
    def b_do!(key, type = RUN_BSON::BSON_RUBY::OBJECT) # Append object/array element unfinished
      put(type)
      RUN_BSON::BSON_RUBY.serialize_cstr(self, key)
      @b_pos ||= [0]
      @a_index ||= [0]
    ...
    def b_do(key, type = RUN_BSON::BSON_RUBY::OBJECT) # Append object/array element finished
      @b_pos ||= [0]
    ...
    def doc!(key) # Append object element unfinished
      b_do!(key, RUN_BSON::BSON_RUBY::OBJECT)
    end
    ...
    def array!(key) # Append array element unfinished
      b_do!(key, RUN_BSON::BSON_RUBY::ARRAY)
    end
