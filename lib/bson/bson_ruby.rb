...
module RUN_BSON
  ...
  class BSON_RUBY
    ...
    def serialize(obj, check_keys=false, move_id=false)
      raise(InvalidDocument, "RUN_BSON.serialize takes a Hash but got a #{obj.class}") unless obj.is_a?(Hash)
    ...
    def deserialize_regex_data(buf, opts={})
      ...
      bson_regex = RUN_BSON::Regex.new(str, options_str)
    ...
    def serialize_regex_element(buf, key, val)
      ...
      if val.is_a?(RUN_BSON::Regex)
        options_str << 'i' if ((options & RUN_BSON::Regex::IGNORECASE) != 0)
        options_str << 'l' if ((options & RUN_BSON::Regex::LOCALE_DEPENDENT) != 0)
        options_str << 'm' if ((options & RUN_BSON::Regex::MULTILINE) != 0)
        options_str << 's' if ((options & RUN_BSON::Regex::DOTALL) != 0)
        options_str << 'u' if ((options & RUN_BSON::Regex::UNICODE) != 0)
        options_str << 'x' if ((options & RUN_BSON::Regex::EXTENDED) != 0)
      else
