...
module Mongo
  ...
  class CollectionWriter
        ...
        until docs.empty? || (!errors.empty? && !collect_on_error) # process documents a batch at a time
          ...
          while !docs.empty? && batch_docs.size < @max_write_batch_size
            begin
              ...
              serialized_doc ||= RUN_BSON::BSON_CODER.serialize(doc, check_keys, true, max_serialize_size)
            rescue RUN_BSON::InvalidDocument, RUN_BSON::InvalidKeyName, RUN_BSON::InvalidStringEncoding => ex
    ...
    def send_bulk_write_command(op_type, documents, check_keys, opts, collection_name=@name)
      if op_type == :insert
        ...
        documents.each do |doc|
          ...
          if check_keys
            doc.each_key do |key|
              key = key.to_s
              raise RUN_BSON::InvalidKeyName.new("key #{key} must not start with '$'") if key[0] == ?$
              raise RUN_BSON::InvalidKeyName.new("key #{key} must not contain '.'") if key.include? ?.
            end
          end
        end
      ...
      request = RUN_BSON::OrderedHash[op_type, collection_name].merge!(
  ...
  class CollectionOperationWriter < CollectionWriter
    ...
    def send_write_operation(op_type, selector, doc_or_docs, check_keys, opts, write_concern, collection_name=@name)
      message = RUN_BSON::ByteBuffer.new("", @connection.max_message_size)
      message.put_int((op_type == :insert && !!opts[:continue_on_error]) ? 1 : 0)
      RUN_BSON::BSON_RUBY.serialize_cstr(message, "#{@db.name}.#{collection_name}")
      if op_type == :update
        update_options  = 0
      ...
      message.put_binary(RUN_BSON::BSON_CODER.serialize(selector, false, true, @connection.max_bson_size).to_s) if selector
      [doc_or_docs].flatten(1).compact.each do |document|
        message.put_binary(RUN_BSON::BSON_CODER.serialize(document, check_keys, true, @connection.max_bson_size).to_s)
        if message.size > @connection.max_message_size
          raise RUN_BSON::InvalidDocument, "Message is too large. This message is limited to #{@connection.max_message_size} bytes."
        end
      end
    ...
    def batch_message_initialize(message, op_type, continue_on_error, write_concern)
      ...
      RUN_BSON::BSON_RUBY.serialize_cstr(message, "#{@db.name}.#{@name}")
    end
  ...
  class CollectionCommandWriter < CollectionWriter
    ...
    def send_write_command(op_type, selector, doc_or_docs, check_keys, opts, write_concern, collection_name=@name)
      ...
      request = RUN_BSON::OrderedHash[op_type, collection_name, WRITE_COMMAND_ARG_KEY[op_type], argument]
    ...
    def batch_message_initialize(message, op_type, continue_on_error, write_concern)
      message.clear!.clear
      @bson_empty ||= RUN_BSON::BSON_CODER.serialize({})
    ...
    def batch_message_send(message, op_type, batch_docs, write_concern, continue_on_error)
      message.finish!
      request = RUN_BSON::OrderedHash[op_type, @name, :bson, message]
