...
module Mongo
  ...
  class Cursor
    ...
    def count(skip_and_limit = false)
      check_command_cursor
      command = RUN_BSON::OrderedHash["count",  @collection.name, "query",  @selector]

      if skip_and_limit
        command.merge!(RUN_BSON::OrderedHash["limit", @limit]) if @limit != 0
        command.merge!(RUN_BSON::OrderedHash["skip", @skip]) if @skip != 0
      end
      ...
      command.merge!(RUN_BSON::OrderedHash["fields", @fields])
      command.merge!(RUN_BSON::OrderedHash["hint", hint]) if hint
    ...
    def close
      if @cursor_id && @cursor_id != 0
        message = RUN_BSON::ByteBuffer.new([0, 0, 0, 0])
        message.put_int(1)
    ...
    def send_get_more
      ...
      # DB name.
      RUN_BSON::BSON_RUBY.serialize_cstr(message, "#{@db.name}.#{@collection.name}")
    ...
    def construct_query_message
      message = RUN_BSON::ByteBuffer.new("", @connection.max_bson_size + MongoClient::COMMAND_HEADROOM)
      ...
      if query_contains_special_fields? && @bson # costs two serialize calls
        query_message = RUN_BSON::BSON_CODER.serialize(@selector, false, false, @connection.max_bson_size + MongoClient::APPEND_HEADROOM)
        query_message.grow(@bson)
        query_spec = construct_query_spec
        query_spec.delete('$query')
        query_message.grow(RUN_BSON::BSON_CODER.serialize(query_spec, false, false, @connection.max_bson_size))
      else # costs only one serialize call
        spec = query_contains_special_fields? ? construct_query_spec : @selector
        spec.merge!(@opts)
        query_message = RUN_BSON::BSON_CODER.serialize(spec, false, false, @connection.max_bson_size + MongoClient::APPEND_HEADROOM)
        query_message.grow(@bson) if @bson
      end
      message.put_binary(query_message.to_s)
      message.put_binary(RUN_BSON::BSON_CODER.serialize(@fields, false, false, @connection.max_bson_size).to_s) if @fields
      message
    end
