...
module Mongo
  ...
  class GridIO
    ...
    def create_chunk(n)
      chunk = RUN_BSON::OrderedHash.new
      chunk['_id']      = RUN_BSON::ObjectId.new
      chunk['n']        = n
    ...
    def write_string(string)
      ...
      while (to_write > 0) do
        ...
        @current_chunk['data'] = RUN_BSON::Binary.new((@current_chunk['data'].to_s << string[-to_write, step_size]).unpack("c*"))
    ...
    def init_write(opts)
      opts           = opts.dup
      @files_id      = opts.delete(:_id) || RUN_BSON::ObjectId.new
    ...
    def get_md5
      md5_command            = RUN_BSON::OrderedHash.new
