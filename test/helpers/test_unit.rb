...
class Test::Unit::TestCase
  ...
  def perform_step_down(member)
    ...
    timeout = 20 # seconds
    begin
      step_down_command = RUN_BSON::OrderedHash.new
  ...
  def match_document(key, expected, actual) # special cases for Regexp match, RUN_BSON::ObjectId, Range
    ...
    elsif expected.is_a?(RUN_BSON::ObjectId) && actual.is_a?(RUN_BSON::ObjectId)
  ...
  def grant_admin_user_eval_role(client)
    if auth_enabled?(client) && client.server_version >= "2.6"
      ...
      admin = client['admin']
      any_priv = RUN_BSON::OrderedHash.new
      any_priv[:resource] = { :anyResource => true }
      any_priv[:actions] = ['anyAction']

      create_role = RUN_BSON::OrderedHash.new
      ...
      grant_role = RUN_BSON::OrderedHash.new
