...
class BSONTest < Test::Unit::TestCase
  ...
  def test_interface
    doc = { 'a' => 1 }
    bson = RUN_BSON.serialize(doc)
    assert_equal doc, RUN_BSON.deserialize(bson)
  end
  ...
  def test_read_bson_document
    ...
    bson = RUN_BSON.read_bson_document(strio)
    doc = {"_id"=>RUN_BSON::ObjectId('5115883c3d75c94d3aa18b63'), "a"=>1.0}
  ...
  def test_bson_ruby_dbref_not_used
    ...
    RUN_BSON::BSON_RUBY.serialize_cstr(buf, val)
  ...
  def test_require_hash
    assert_raise_error InvalidDocument, "takes a Hash" do
      RUN_BSON.serialize('foo')
    end

    assert_raise_error InvalidDocument, "takes a Hash" do
      RUN_BSON.serialize(Object.new)
    end

    assert_raise_error InvalidDocument, "takes a Hash" do
      RUN_BSON.serialize(Set.new)
    end
  end
  ...
  def test_limit_max_bson_size
    doc = {'name' => 'a' * RUN_BSON::DEFAULT_MAX_BSON_SIZE}
  ...
  if RUBY_VERSION < '1.9'
    ...
    unless RUBY_PLATFORM == 'java'
      def test_non_utf8_string
        assert_raise RUN_BSON::InvalidStringEncoding do
          RUN_BSON::BSON_CODER.serialize({'str' => 'aé'.encode('iso-8859-1')})
        end
      end
      ...
      def test_invalid_utf8_string
        ...
        assert_raise RUN_BSON::InvalidStringEncoding do
          RUN_BSON::BSON_CODER.serialize({'str' => str})
        end
      end

      def test_non_utf8_key
        assert_raise RUN_BSON::InvalidStringEncoding do
          RUN_BSON::BSON_CODER.serialize({'aé'.encode('iso-8859-1') => 'hello'})
        end
      end
      ...
      def test_default_internal_is_honored
        ...
        str = "壁に耳あり、障子に目あり"
        bson = RUN_BSON::BSON_CODER.serialize("x" => str)

        silently { Encoding.default_internal = 'EUC-JP' }
        out = RUN_BSON::BSON_CODER.deserialize(bson)["x"]
  ...
  def test_code
    ...
    assert_match /<RUN_BSON::Code:.*@data="this.a.b < this.b".*>/, code.inspect
    doc = {'$where' => code}
    assert_doc_pass(doc)
    code = 'this.c.d < this.e'.to_bson_code # core_ext.rb
    assert_equal RUN_BSON::Code, code.class
    assert_equal code, code.to_bson_code
  end

  def test_code_with_symbol
    assert_raise_error ArgumentError, "RUN_BSON::Code must be in the form of a String" do
      Code.new(:fubar)
    end
  end
  ...
  def test_bson_regex
    doc = { 'doc' => RUN_BSON::Regex.new('foobar') }
    bson = @encoder.serialize(doc)
    assert_equal @encoder.serialize(doc).to_a, bson.to_a
  ...
  def test_bson_regex_with_nonruby_flags
    ...
    bson_regex = RUN_BSON::Regex.new('foobar', 'i', 'l', 'm', 's', 'u', 'x')
    doc = { 'regexp' => bson_regex }
    bson = @encoder.serialize(doc)
    assert_equal @encoder.serialize(doc).to_a, bson.to_a
    bson_regex = @encoder.deserialize(bson, :compile_regex => false)['regexp']
    assert_equal RUN_BSON::Regex::IGNORECASE,       RUN_BSON::Regex::IGNORECASE       & bson_regex.options
    assert_equal RUN_BSON::Regex::LOCALE_DEPENDENT, RUN_BSON::Regex::LOCALE_DEPENDENT & bson_regex.options
    assert_equal RUN_BSON::Regex::MULTILINE,        RUN_BSON::Regex::MULTILINE        & bson_regex.options
    assert_equal RUN_BSON::Regex::DOTALL,           RUN_BSON::Regex::DOTALL           & bson_regex.options
    assert_equal RUN_BSON::Regex::UNICODE,          RUN_BSON::Regex::UNICODE          & bson_regex.options
    assert_equal RUN_BSON::Regex::EXTENDED,         RUN_BSON::Regex::EXTENDED         & bson_regex.options
  end
  ...
  def test_ruby_regexp_to_bson_regex
    ...
    assert_equal RUN_BSON::Regex::MULTILINE,  RUN_BSON::Regex::MULTILINE  & bson_regx.options
    assert_equal RUN_BSON::Regex::EXTENDED,   RUN_BSON::Regex::EXTENDED   & bson_regx.options
    assert_equal RUN_BSON::Regex::IGNORECASE, RUN_BSON::Regex::IGNORECASE & bson_regx.options
    assert_equal RUN_BSON::Regex::DOTALL,     RUN_BSON::Regex::DOTALL     & bson_regx.options
  end
  ...
  def test_exeption_on_using_unsupported_date_class
    [DateTime.now, Date.today, Zone].each do |invalid_date|
      ...
      begin
      RUN_BSON::BSON_CODER.serialize(doc)
      rescue => e
      ensure
  ...
  def test_put_id_first
    val = RUN_BSON::OrderedHash.new
    val['not_id'] = 1
    val['_id'] = 2
    roundtrip = @encoder.deserialize(@encoder.serialize(val, false, true).to_s)
    assert_kind_of RUN_BSON::OrderedHash, roundtrip
  ...
  def test_bad_id_keys
    doc = { '_id' => { '$bad' => 123 } }
    check_keys = true
    assert_raise RUN_BSON::InvalidKeyName do
  ...
  def test_move_id_with_nested_doc
    b = RUN_BSON::OrderedHash.new
    b['text'] = 'abc'
    b['_id']   = 2
    c = RUN_BSON::OrderedHash.new
    c['text'] = 'abc'
    c['hash'] = b
    c['_id']  = 3
  ...
  def test_invalid_key_names
    ...
    assert_raise RUN_BSON::InvalidDocument do
