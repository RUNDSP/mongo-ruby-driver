...
class BulkWriteCollectionViewTest < Test::Unit::TestCase
  ...
  context "Bulk API CollectionView" do
    ...
    should "handle single upsert - spec Handling upserts" do # chose array always for upserted value
      with_write_commands_and_operations(@db.connection) do |wire_version|
        @collection.remove
        @collection.ensure_index(RUN_BSON::OrderedHash[:a, Mongo::ASCENDING], {:unique => true})
        bulk = @collection.initialize_ordered_bulk_op
        ...
        assert_match_document(
            {
                ...
                "upserted" => [
                    {"_id" => RUN_BSON::ObjectId('52a16767bb67fbc77e26a310'), "index" => 0}
                ]
            }, result, "wire_version:#{wire_version}")
      end
    end
        ...
        assert_match_document(
            {
                ...
                "nUpserted" => 1,
                "upserted" => [
                    {"_id" => RUN_BSON::ObjectId('52a16767bb67fbc77e26a310'), "index" => 1}
                ]
            }, result, "wire_version:#{wire_version}")
        ...
        assert_match_document(
            {
                ...
                "upserted" => [
                    {
                        "index" => 10,
                        "_id" => RUN_BSON::ObjectId('52a1e4a4bb67fbc77e26a340')
                    }
                ]
            }, result, "wire_version:#{wire_version}")
        ...
        assert_match_document(
            {
                ...
                "nModified" => batch_commands?(wire_version) ? 0 : nil,
                "upserted" => [
                    {"index" => 0, "_id" => RUN_BSON::ObjectId('52a1e37cbb67fbc77e26a338')},
                    {"index" => 1, "_id" => RUN_BSON::ObjectId('52a1e37cbb67fbc77e26a339')}
                ]
            }, result, "wire_version:#{wire_version}")
      end
    end
